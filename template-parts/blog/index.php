...
<div class="w-full overflow-hidden site-main max-sm:mt-[2.5rem] mt-[5rem]" x-data="{
  ...
  <section id="featured-posts" class="flex overflow-hidden relative bg-primary">
    <div <?php echo $section_style; ?> class="flex flex-col items-center mx-auto w-full bg-primary">
      <div class="overflow-hidden relative max-w-[1408px] max-xxl:px-[1rem] w-full hero-background">
        ...
        <div class="flex overflow-hidden z-0 flex-wrap gap-6 items-end pb-14 w-full max-md:max-w-full">
          ...
          <div class="flex-1 pb-2 text-base shrink min-w-60 max-md:max-w-full">
            ...
            <div role="radiogroup" aria-label="Filter news by category" class="flex flex-wrap gap-4 items-start mt-2 w-full font-medium max-md:max-w-full">
              <button
                role="radio"
              ...
              ?>
                <button
                  ...
                  aria-checked="<?php echo esc_attr($checked); ?>"
                  tabindex="<?php echo esc_attr($tab); ?>"
                >
                  <?php echo $name; ?>
                </button>
              <?php endforeach; ?>
    ...
    <div class="flex flex-col gap-8 pt-12 pb-14 w-full bg-white max-md:p-8 max-sm:p-4">
      ...
      <div class="flex justify-between items-center w-full">
        <span class="text-2xl font-bold leading-7 text-slate-600">
          <?php echo esc_html((string) wp_count_posts()->publish); ?> posts
        </span>
      ...
      <!-- FEATURED POSTS WRAPPER (add your featured posts markup inside this) -->
      <div id="featured-posts">
        <?php /* Featured posts output goes here. Keep your existing featured markup unchanged. */ ?>
      </div>
      ...
      <main class="w-full" role="main" aria-label="Blog posts">
        <div class="flex flex-col">
            ...
            if ($query->have_posts()) :
              ...
              while ($query->have_posts()) : $query->the_post();
                ...
                if (($post_count - 1) % $posts_per_row === 0) {
                  ...
                  echo '<div class="flex flex-wrap gap-10 items-start' . esc_attr($margin_class) . ' max-md:max-w-full">';
                }

                $post_cats = array_map(function ($c) {
                  return $c->slug;
                }, get_the_category());
          ...
          <!-- Pagination Component (SVG icons) -->
              ...
              <div class="flex gap-5 justify-between items-center">
                ...
                <?php if ($prev_page >= 1) : ?>
                  <a
                    ...
                    class="flex justify-center items-center w-[22px] h-[22px] transition-opacity hover:opacity-70 focus:outline-2 focus:outline-blue-600 focus:outline-offset-2 btn"
                    aria-label="Go to previous page"
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" width="10" height="18" viewBox="0 0 10 18" fill="none" aria-hidden="true">
                      <path d="M9 17L1 9L9 1" stroke="#0902A4" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>
                  </a>
                <?php else : ?>
                  <span class="flex justify-center items-center w-[22px] h-[22px] opacity-30 cursor-not-allowed" aria-label="Previous page not available">
                    <svg xmlns="http://www.w3.org/2000/svg" width="10" height="18" viewBox="0 0 10 18" fill="none" aria-hidden="true">
                ...
                <div class="flex gap-4 self-center text-base leading-3 text-center text-primary whitespace-nowrap w-[78px]">
                  ...
                  for ($i = $start_page; $i <= $end_page && $i <= $max_pages; $i++) :
                    if ($i === $current_page) : ?>
                      <span class="font-bold text-blue-950" aria-current="page" aria-label="Current page, page <?php echo esc_attr((string) $i); ?>">
                    ...
                    <?php else : ?>
                      <a
                        ...
                        aria-label="Go to page <?php echo esc_attr((string) $i); ?>"
                      >
                        <?php echo esc_html((string) $i); ?>
                      </a>
                    <?php endif;
                ...
                <?php else : ?>
                  <span class="flex justify-center items-center w-[22px] h-[22px] opacity-30 cursor-not-allowed" aria-label="Next page not available">
                    <svg xmlns="http://www.w3.org/2000/svg" width="10" height="18" viewBox="0 0 10 18" fill="none" aria-hidden="true">
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // INITIAL STATE: read from your existing aria-checked
  const initiallyChecked = document.querySelector('.filter-btn[aria-checked="true"]');
  let activeFilter = (initiallyChecked && initiallyChecked.dataset.filter) ? initiallyChecked.dataset.filter : 'all';
  ...
  function cardVisible(card) {
    ...
    // NOTE: your current markup puts <h3> inside .sr-only, which is fine (still in DOM)
  ...
  function applyFilter() {
    ...
    if (featuredPosts) {
      featuredPosts.classList.toggle('hidden', needsClear);
    }
  }
  ...
  buttons.forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      // Update aria-checked (your markup uses aria-checked, not aria-pressed)
      buttons.forEach(b => {
