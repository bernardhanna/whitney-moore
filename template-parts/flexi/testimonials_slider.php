      ...
      <div class="relative mt-8 w-full max-md:mt-0">
        ...
        <?php if ($enable_slider) : ?>
          ...
          <div class="relative w-full lg:left-5" data-slick-shell="<?php echo esc_attr($section_id); ?>">
            <div class="matrix-slick" id="<?php echo esc_attr($track_id); ?>" data-slick-root="<?php echo esc_attr($section_id); ?>">
              ...
              ?>
                <div class="px-2">
                  <article
                    class="relative h-[520px] overflow-hidden t-card cursor-pointer"
                  ...
                  >
                    ...
                    <img
                      ...
                      class="object-cover relative inset-0 w-full h-full pointer-events-none"
                      loading="lazy"
                      decoding="async"
                    />

                    <!-- Overlay card -->
                    <div class="absolute bottom-0 w-full max-w-[95%] mb-4 pl-5 pr-2 pointer-events-none">
                      <div class="bg-white shadow-[0_8px_24px_rgba(0,0,0,0.15)] px-6 py-5 w-full pointer-events-none">
                        ...
                        <?php if (!empty($permalink)) : ?>
                          <div class="inline-flex gap-2 items-center mt-4 text-primary" aria-hidden="true">
          ...
          jQuery(function($){
            ...
            // Make whole card clickable (and keyboard accessible) without breaking swipe
            var dragging = false;
            var startX = 0, startY = 0;

            $root.on('mousedown touchstart', '.t-card[data-href]', function(e){
              dragging = false;
              var p = e.originalEvent.touches ? e.originalEvent.touches[0] : e;
              startX = p.clientX; startY = p.clientY;
            });

            $root.on('mousemove touchmove', '.t-card[data-href]', function(e){
              var p = e.originalEvent.touches ? e.originalEvent.touches[0] : e;
              if (Math.abs(p.clientX - startX) > 8 || Math.abs(p.clientY - startY) > 8) dragging = true;
            });

            $root.on('click', '.t-card[data-href]', function(e){
              if (dragging) return; // allow swipe without navigating
              var href = this.getAttribute('data-href');
              if (href) window.location.assign(href);
            });

            $root.on('keydown', '.t-card[data-href]', function(e){
        ...
        <?php else : ?>
          ...
          <div class="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-4">
            ...
            ?>
              <article
                class="relative h-[520px] overflow-hidden cursor-pointer"
                <?php if (!empty($permalink)) : ?>
                  role="link"
                  tabindex="0"
                  data-href="<?php echo esc_url($permalink); ?>"
              ...
              >
                ...
                <div class="absolute bottom-6 left-6 pointer-events-none">
                  <div class="bg-white shadow-[0_8px_24px_rgba(0,0,0,0.15)] px-6 py-5 w-[320px] max-w-[85vw] pointer-events-none">
                    <?php if ($show_name && !empty($title)) : ?>
                      <div class="text-base font-semibold tracking-normal text-black">
                        <?php echo esc_html($title); ?>
                      </div>
          ...
          <script>
          // Click + keyboard for non-slider cards
          (function(){
            var root = document.getElementById(<?php echo wp_json_encode($section_id); ?>);
            if (!root) return;

            root.addEventListener('click', function(e){
              var card = e.target.closest('.t-card[data-href], article[data-href]');
              if (!card) return;
