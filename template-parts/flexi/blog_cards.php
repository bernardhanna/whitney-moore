 ...
 * - Pulls the latest posts (default 3)
 * - Uses get_sub_field() everywhere
 * - Tailwind only (no aspect-* / min-w-* utilities)
 */
...
// CTAs (ACF link arrays are preferred; fallback to post permalink)
$small_cta          = get_sub_field('small_cta'); // link array (optional override)
$small_cta_text     = get_sub_field('small_cta_text') ?: 'Discover';

$big_cta            = get_sub_field('big_cta');   // link array (optional override)
...
// simple helper: first category name
...
if ($query->have_posts()) {
    ...
    while ($query->have_posts()) {
        $query->the_post();
        if ($i < 2) { $left_posts[]  = get_post(); }  // first two small
        else        { $right_posts[] = get_post(); }  // third big (and any extras)
        $i++;
    }
}
wp_reset_postdata();
            ...
            <div class="max-w-[1728px] mx-auto flex flex-col lg:flex-row items-start gap-8">
                            ...
                            <article class="relative h-[332px] rounded-lg overflow-hidden group">
                                ...
                                if ($image) {
                                    echo wp_get_attachment_image(
                                        ...
                                        'large',
                                        false,
                                        ['alt' => esc_attr($img_alt), 'class' => 'absolute inset-0 w-full h-full object-cover', 'loading' => 'lazy']
                                    );
                                }
                                ?>
                                <div class="absolute bottom-6 left-6 right-6 <?php echo esc_attr($overlay_blur_class . ' ' . $overlay_bg_class); ?> p-6 flex flex-col gap-6 rounded-md">
                                    ...
                                    <?php
                                    // prefer ACF link array if provided, else fall back to post permalink
                                    ...
                                    ?>
                                    <a href="<?php echo $href; ?>" class="<?php echo esc_attr($link_color_class); ?> font-work-sans text-base font-medium underline transition-colors"<?php echo $target; ?>>
                                        <?php echo $label; ?>
                                    </a>
                                </div>
                            </article>
                ...
                <div class="flex-1">
                    <?php if (!empty($right_posts)) : ?>
                        <?php foreach ($right_posts as $post_obj) :
                            $pid     = $post_obj->ID;
                            $title   = get_the_title($pid);
                            $link    = get_permalink($pid);
                            $image   = get_post_thumbnail_id($pid);
